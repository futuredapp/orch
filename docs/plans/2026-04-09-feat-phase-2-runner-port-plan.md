...
8. **Default-deny env contract.** **Superseded by [2026-04-27 env passthrough plan](2026-04-27-feat-env-passthrough-plan.md).** The original "default-deny + loader scrub" was never implemented in `runRunner`; the new contract is passthrough — every process orch launches sees the same env the orch process saw, with `ctx.env` overriding on conflict. Original archaeology preserved below for context. ~~`runRunner` must NOT spread `process.env`. Document the contract: `buildCommand` returns the exact env the subprocess sees, plus `runRunner` strips loader-injection vars (`LD_PRELOAD`, `LD_LIBRARY_PATH`, `LD_AUDIT`, `DYLD_*`, `NODE_OPTIONS`, `BUN_INSPECT*`, `PYTHONSTARTUP`, `PERL5OPT`, `RUBYOPT`) as belt-and-suspenders.~~
