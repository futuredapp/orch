...
// Enter on a past step registers a `file-tail` source in a per-source tmux
// session and swaps the visible right pane to it — no `respawnPane` on the
// visible pane anywhere on the path.
...
describe('right-pane-controller swap-based replay (U8)', () => {
  it('spawns tail -F over .replay/<step>.txt in a per-source session and swaps in for a commit step', async () => {
    ...
    const SOCKET = socketName('orch-int-1')
    ...
    expect(create.opts.session).toBe('orch-src-replay-commit-c1')
    expect(create.opts.socket).toBe(SOCKET)
    ...
    expect(command[1]).toBe('-n')
    expect(command[2]).toBe('5000')
    expect(command[3]).toBe('-F')
    expect(command[4]).toMatch(/\.replay\/commit:c1\.txt$/)
  ...
  it('tails .replay/<step>.txt for an autonomous step with no persisted tee (JSON fallback)', async () => {
    ...
    expect(command[4]).toMatch(/\.replay\/plan\.txt$/)
  ...
  it('warm-caches the replay pane: re-Enter on the same step swaps without re-spawning', async () => {
    const tmux = new FakeTmuxService()
    tmux.nextCreateSessionPaneId(paneId('%150'))
    const stateDir = `${tempDir}/state`
    await mkdir(stateDir, { recursive: true })
    ...
    controller.onIntent({ type: 'enter', stepName: 'commit:c1' })
    await flush()
    tmux.nextCreateSessionPaneId(paneId('%151'))
    controller.onIntent({ type: 'enter', stepName: 'commit:c2' })
    await flush()
    // Re-enter c1 — must reuse the warm-cached pane, no new createSession.
    controller.onIntent({ type: 'enter', stepName: 'commit:c1' })
    await flush()
  ...
  it('swaps to placeholder on follow-live after a prior Enter when no live/rollup is registered', async () => {
    ...
    tmux.nextCreateSessionPaneId(paneId('%399'))
