...
describe.skipIf(!tmuxAvailable)('mountTmuxHost end-to-end with FakeRunner', () => {
  it('exposes left and right pane handles for the orch session', async () => {
  ...
  it('captures pane text with ANSI stripped by default and raw bytes via captureRaw', async () => {
    const fixture = await createRealTmuxFixture({ env: {} })
    fixturesToDispose.push(fixture)
    const harness = await mountTmuxHost(fixture, { disableStepsView: true })
    harnessesToTeardown.push(harness)
  ...
  it('runWorkflow drives a single FakeRunner step to completion (empty workflow case)', async () => {
    const fixture = await createRealTmuxFixture({ env: {} })
    fixturesToDispose.push(fixture)
    const harness = await mountTmuxHost(fixture, { disableStepsView: true })
    harnessesToTeardown.push(harness)
  ...
  it('runWorkflow uses the agentProcessService slot so a scripted FakeRunner runs end-to-end', async () => {
    const fixture = await createRealTmuxFixture({ env: {} })
    fixturesToDispose.push(fixture)
...
describe.skipIf(!tmuxAvailable)('PaneHandle.waitForText timeout shape', () => {
  it('rejects with an error containing the last frame when the text never appears', async () => {
    const fixture = await createRealTmuxFixture({ env: {} })
    fixturesToDispose.push(fixture)
    const harness = await mountTmuxHost(fixture, { disableStepsView: true })
    harnessesToTeardown.push(harness)
...
describe.skipIf(!tmuxAvailable)('PaneHandle.waitFor predicate shape', () => {
  it('resolves on the first predicate match without exhausting the timeout', async () => {
    const fixture = await createRealTmuxFixture({ env: {} })
    fixturesToDispose.push(fixture)
    const harness = await mountTmuxHost(fixture, { disableStepsView: true })
    harnessesToTeardown.push(harness)
...
describe.skipIf(!tmuxAvailable)('sendKeys named-key dispatch', () => {
  it('sends Enter as a real keystroke (tmux exits 0)', async () => {
    const fixture = await createRealTmuxFixture({ env: {} })
    fixturesToDispose.push(fixture)
    const harness = await mountTmuxHost(fixture, { disableStepsView: true })
    harnessesToTeardown.push(harness)
  ...
  it('sends a literal F via the service path so the keymap reads it as uppercase-F', async () => {
    const fixture = await createRealTmuxFixture({ env: {} })
    fixturesToDispose.push(fixture)
    const harness = await mountTmuxHost(fixture, { disableStepsView: true })
    harnessesToTeardown.push(harness)
