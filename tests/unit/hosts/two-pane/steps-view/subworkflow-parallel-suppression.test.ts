...
describe('projectStepsView — parallel suppression (AE9, AE13)', () => {
  it('suppresses sub boundary rows for sub-of-parallel branches (AE9)', () => {
    ...
    const subOverlay = new Map<string, SubworkflowOverlay>([
      ['shipA', { status: 'completed', depth: 1, insideParallel: true, startedAt: 0, endedAt: 1 }],
      ['shipB', { status: 'completed', depth: 1, insideParallel: true, startedAt: 0, endedAt: 1 }],
      ['shipC', { status: 'completed', depth: 1, insideParallel: true, startedAt: 0, endedAt: 1 }],
    ])
  ...
  it('suppresses transitively for sub-of-sub-inside-parallel (AE13)', () => {
    ...
    const subOverlay = new Map<string, SubworkflowOverlay>([
      ['outer', { status: 'completed', depth: 1, insideParallel: true, startedAt: 0, endedAt: 1 }],
      ['inner', { status: 'completed', depth: 2, insideParallel: true, startedAt: 0, endedAt: 1 }],
    ])
  ...
  it('does NOT suppress sequential subs whose steps run outside any parallel', () => {
    ...
    const subOverlay = new Map<string, SubworkflowOverlay>([
      ['simple', { status: 'completed', depth: 1, startedAt: 0, endedAt: 1, durationMs: 1 }],
    ])
