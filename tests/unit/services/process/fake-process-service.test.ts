...
describe.skip('FakeProcessService', () => {
  ...
  it('emits the scripted stderr lines symmetrically with stdout', async () => {
    const fake = new FakeProcessService()
    fake.when(['cmd']).respondWith({ stdout: ['out'], stderr: ['err1', 'err2'], exitCode: 0 })
  ...
  it('serves two distinct argvs independently with no cross-talk', async () => {
    const fake = new FakeProcessService()
    fake.when(['a']).respondWith({ stdout: ['alpha'], exitCode: 0 })
    fake.when(['b']).respondWith({ stdout: ['beta'], exitCode: 1 })
  ...
  it('consumes scripts for the same argv in FIFO order', async () => {
    const fake = new FakeProcessService()
    fake.when(['cmd']).respondWith({ stdout: ['first'], exitCode: 0 })
    fake.when(['cmd']).respondWith({ stdout: ['second'], exitCode: 0 })
  ...
  it('reports exitCode -1 via wait() when kill() interrupts the iteration with an AbortError', async () => {
    const fake = new FakeProcessService()
    fake.when(['long']).respondWith({ stdout: ['a', 'b', 'c', 'd', 'e'], exitCode: 0 })
  ...
  it('does not resolve wait() before the stdout iterator has emitted all scripted lines', async () => {
    const fake = new FakeProcessService()
    fake.when(['cmd']).respondWith({ stdout: ['line1', 'line2', 'line3'], exitCode: 0 })
