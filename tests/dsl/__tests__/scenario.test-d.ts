...
import { type SharedApp, scenario } from '../index.ts'
...
export async function _scenarioTypeTests(): Promise<void> {
  ...
  scenario(
    { name: 'no-complete', drivers: ['model'], feature: 'f', oldTestRefs: [] },
    async (app) => {
  ...
  scenario(
    { name: 'resize-ok', drivers: ['screen'], feature: 'f', oldTestRefs: [] },
    async (app) => {
      await app.resize(80, 24)
      await app.leftPane.assertQuitHintVisible()
    },
  ...
  scenario(
    { name: 'press-ok', drivers: ['lifecycle'], feature: 'f', oldTestRefs: [] },
    async (app) => {
