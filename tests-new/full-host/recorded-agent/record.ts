...
import { BunClock, BunProcessService, FakeProcessService, path } from '../../../src/services/index.ts'
import {
...
async function record(scenarioId: string, runnerName: 'claude' | 'codex', prompt: string): Promise<void> {
...
function readCassetteFile(scenarioId: string): RecordedAgentCassette {
  const raw = JSON.parse(readFileSync(join(CASSETTES_DIR, `${scenarioId}.json`), 'utf-8')) as unknown
...
if (import.meta.main) {
  ...
  if (process.argv.includes('--verify')) {
    ...
    process.stdout.write(result.ok ? `verify ok: ${scenarioId}\n` : `verify FAILED: ${result.reason}\n`)
