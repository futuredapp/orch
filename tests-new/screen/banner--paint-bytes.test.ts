...
scenario(
  ...
  async (app) => {
    ...
    await app.launch({ steps: ['plan'], stopAt: 'mid-step', banner: { kind: 'info', text: 'saved to disk' } })
    ...
    await app.launch({ steps: ['plan'], stopAt: 'mid-step', banner: { kind: 'error', text: 'disk full' } })
